      ...
      try {
          ...
          generationConfig: {
            ...
            responseSchema: {
              ...
              properties: {
                thought: {
                  type: "string",
                  description: "Brainstorm 3 D&D settings and choose one hook.",
                },
                premise: { type: "string" },
              },
        ...
        if (data.candidates && data.candidates[0].content.parts) {
          ...
          if (parsedData.premise) {
            // We set the systemPrompt to the premise generated by the AI
            systemPrompt.value = parsedData.premise.trim();
          }
        }
      } catch (err) {
        if (err.name === "AbortError") {
          alert("The DM is taking too long to prep the world—try again!");
        } else {
