...
1. THOUGHT PROCESS: Use the "thought" field for internal strategy and diagnosis ONLY (e.g., "Student is struggling with unit conversion, I will explain mcg to mg"). Do NOT put the actual message or dialogue intended for the student here.
2. ONE STEP AT A TIME: Put all student-facing dialogue and questions in the "response" field. End your "response" with exactly ONE clear question.
...
createApp({
  setup() {
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Summarize the following chronological excerpt of a nursing school tutoring session into a single, concise progress paragraph.
                  Focus on:
                  1. What nursing concepts were introduced.
    ...
    onMounted(async () => {
      if (localStorage.getItem("medchat_api_key"))
        apiKey.value = localStorage.getItem("medchat_api_key");
      if (localStorage.getItem("medchat_model"))
        selectedModel.value = localStorage.getItem("medchat_model");
      if (localStorage.getItem("medchat_tts_model"))
        selectedTTSModel.value = localStorage.getItem("medchat_tts_model");
      if (localStorage.getItem("medchat_randomizer_model"))
        randomizerModel.value = localStorage.getItem(
          "medchat_randomizer_model",
        );
      if (localStorage.getItem("medchat_tts_voice"))
        selectedVoice.value = localStorage.getItem("medchat_tts_voice");
      if (localStorage.getItem("medchat_tts_prosody"))
        ttsProsodyNudge.value = localStorage.getItem("medchat_tts_prosody");
      if (localStorage.getItem("medchat_system_prompt"))
        systemPrompt.value = localStorage.getItem("medchat_system_prompt");
      if (localStorage.getItem("medchat_summary_batch"))
        summaryBatchSize.value = parseInt(
          localStorage.getItem("medchat_summary_batch"),
        );
    ...
    const saveAllSettings = () => {
      const oldRules = localStorage.getItem("medchat_system_prompt") || "";
      const rulesChanged = oldRules.trim() !== systemPrompt.value.trim();

      localStorage.setItem("medchat_api_key", apiKey.value);
      localStorage.setItem("medchat_model", selectedModel.value);
      // localStorage.setItem(
      //   "fractionchat_randomizer_model",
      //   randomizerModel.value,
      // );
      localStorage.setItem("medchat_system_prompt", systemPrompt.value);
      localStorage.setItem("medchat_tts_model", selectedTTSModel.value);
      localStorage.setItem("medchat_tts_voice", selectedVoice.value);
      localStorage.setItem("medchat_tts_prosody", ttsProsodyNudge.value);
      localStorage.setItem("medchat_summary_batch", summaryBatchSize.value);
      ...
      if (messages.value.length === 0 && apiKey.value) {
        ...
        if (
          confirm(
            "Rules updated! Would you like to restart now to apply these changes?",
          )
        ) {
          db.chats.clear();
          db.facts.clear();
    ...
    const startOver = async () => {
      if (
        !confirm(
          "Are you sure? This will permanently delete everything AND all remembered facts.",
        )
      )
        return;
    ...
    const initializeStory = async () => {
      ...
      const firstMessage =
        ...
        "Welcome to your Med Chat! What medical topic would you like to explore today?";
