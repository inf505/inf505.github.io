    ...
    <div class="class-selection">
      ...
      <div v-if="creationStore.classes && creationStore.classes.length > 0" class="class-selection__grid">
        <div 
          ...
          :style="{ backgroundImage: 'url(https://inf505.github.io/aisf/frontend/images/' + classItem.name.toLowerCase() + '.png)' }"
        >
    ...
    <div class="class-selection">
      <div v-if="creationStore.classes && creationStore.classes.length > 0" class="class-selection__grid">
        ...
        >
          <div class="class-selection__text-content">
            <div>
              <h3 class="class-card__title">{{ classItem.class }}</h3>
              <p v-if="classItem.description" class="class-card__description">
                {{ classItem.description }}
              </p>
            </div>
            <span class="btn btn-primary class-card__button">Select Background</span>
          </div>
        </div>
      </div>
      
      <div v-else>
        <p class="help-text">No character backgrounds available. This may indicate a server issue.</p>
      </div>
    </div>
  `,
};
